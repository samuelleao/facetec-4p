...
var DeveloperStatusMessages = /** @class */ (function () {
  ...
  DeveloperStatusMessages.logInitializeResult = function () {
    ...
    var displayMessage =
      FaceTecStatusEnumFriendlyText.descriptionForFaceTecSDKStatus(
        FaceTecSDK.getStatus()
      );
    // Message to log for developer
    var logMessage = "FaceTecSDK.getStatus(): "
      .concat(FaceTecSDK.getStatus(), ' "')
  ...
  DeveloperStatusMessages.logScanOncompleteResult = function (
    sessionResult,
    idScanResult
  ) {
    // User message to display
    ...
    if (idScanResult != null && idScanResult.status != null) {
      ...
      logMessage = "FaceTecIDScanResult.status: "
        .concat(idScanResult.status, " (")
        .concat(
          FaceTecStatusEnumFriendlyText.descriptionForFaceTecIDScanStatus(
            idScanResult.status
          ),
          ")"
        );
      ...
      if (
        ...
        logMessage += "\n".concat(
          _a.LOG_PREFIX,
          " Developer called idScanResultCallback.cancel before the idScan was successfully processed by the server."
        );
      }
    }
    // Else show the sessionResult
      ...
      if (
        ...
        logMessage += "\n".concat(
          _a.LOG_PREFIX,
          " Developer called FaceScanResultCallback.cancel before the faceScan was successfully processed by the server."
        );
      }
    }
    _a.displayMessage(displayMessage);
